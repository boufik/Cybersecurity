...
| 3  | `nmap -sU <target>` | Yes | Basic | UDP Scan: UDP does **NOT** require a handshake at all. If a port is closed, we expect:<br>`UDP_pkt --> ICMP_type3_code3` |
| 4  | `nmap -sN <target>` | Yes | Advanced | TCP Null Scan: All 6 flags are set to 0. If a port is closed, we expect:<br>`TCP_null --> RST,ACK` |
| 5  | `nmap -sF <target>` | Yes | Advanced | TCP FIN Scan: One flag `FIN` is set to 1. If a port is closed, we expect:<br>`TCP_FIN --> RST,ACK` |
| 6  | `nmap -sX <target>` | Yes | Advanced | TCP Xmas Scan: Three flags `URG`, `PSH` and `FIN` are set to 1. If a port is closed, we expect:<br>`TCP_URG_PSH_FIN --> RST,ACK` |
| 7  | `nmap -sM <target>` | Yes | Advanced | TCP Maimon Scan: Two flags `ACK` and `FIN` are set to 1. **Rather useless scan**. If a port is closed or open, we expect:<br>`TCP_ACK_FIN --> RST` |
| 8  | `nmap -sA <target>` | Yes | Advanced | TCP ACK Scan: One flag `ACK` is set to 1. **This scan exposes the firewall rules, NOT the services**. If a port is closed or open, we expect:<br>`TCP_ACK --> RST` |
| 9  | `nmap -sW <target>` | Yes | Advanced | Similar to TCP ACK scan, but the `Window` field of `RST` packet is examined. **This scan exposes the firewall rules, NOT the services**. If a port is closed or open, we expect:<br>`TCP_ACK --> RST` |
